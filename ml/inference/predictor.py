...
class CodeSensePredictor:
    ...
    def predict(self, code: str, filename: str = "unnamed.py") -> dict:
        ...
        vector = np.array(
            [raw.get(f, 0.0) for f in self.feature_names],
            dtype=np.float32
        ).reshape(1, -1)
    ...
    def _top_signals(self, features: dict, label: str) -> list[str]:
        ...
        for feat in SIGNAL_PRIORITY:
            ...
            if   feat == "docstring_ratio"         and val > 0.6:  signals.append(feat)
            elif feat == "type_annotation_ratio"   and val > 0.6:  signals.append(feat)
            elif feat == "maintainability_index"   and val > 80:   signals.append(feat)
            elif feat == "single_char_var_ratio"   and val < 0.03: signals.append(feat)
            elif feat == "avg_identifier_length"   and val > 8:    signals.append(feat)
            elif feat == "comment_density"         and val > 0.2:  signals.append(feat)
            elif feat == "assertion_count"         and val > 3:    signals.append(feat)
            elif feat == "num_try_except"          and val > 2:    signals.append(feat)
            if len(signals) == 3:
                break
